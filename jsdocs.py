...
def getParser(view):
    ...
    elif sourceLang == 'apex':
        return JsdocsApex(viewSettings)
...
class JsdocsApex(JsdocsParser):
    ...
    def parseFunction(self, line):
        ...
        res = re.search(
            # Modifiers
            r'(?:(public|protected|private|static|abstract|final|transient|synchronized|native|strictfp|global)\s+)*'
            # Return value
            + r'(?P<retval>[a-zA-Z_$][<>., a-zA-Z_$0-9]+(\[\])*)\s+'
            # Method name
            + r'(?P<name>' + self.settings['fnIdentifier'] + r')\s*'
